...
def get_stats(is_simulation: bool, since=None) -> dict:
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            WHERE is_simulation = %s AND status = 'open'
            """,
            [is_simulation],
        )
...
def print_comparison(paper: dict, live: dict, label: str) -> None:
    """Print paper and live stats side by side."""
    print(f"{'=' * 60}")
    print(f"  PAPER vs LIVE — {label}")
    print(f"{'=' * 60}")
    print()
...
def main() -> None:
    ...
    paper = get_stats(is_simulation=True,  since=since)
    live  = get_stats(is_simulation=False, since=since)
    print_comparison(paper, live, label)
